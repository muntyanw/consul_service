...
TEMPLATE_DIR: Final[Path] = Path(__file__).resolve().parent.parent / "data/ui_images"
...
def click_text(
    ...
    """
    OCR-based search: find `query` text on screen and click its center.
    Returns True if clicked, False if not found within `timeout` seconds.

    Parameters
    ----------
    query : str
        Exact substring to look for (case-insensitive) in recognized words.
    timeout : float
        Maximum seconds to keep retrying.
    lang : str
        Tesseract language code, e.g. "eng", "rus", "ukr".
    conf_threshold : float
        Minimal confidence (0.0–1.0) for words to be considered.
    """
    deadline = time.perf_counter() + timeout

    # Ensure pytesseract knows where Tesseract-OCR is (if not in PATH)
    ...
    while time.perf_counter() < deadline:
        ...
        # 2) Run Tesseract OCR with word bounding boxes
        ...
        for i in range(n_boxes):
            ...
            # Convert confidence from “0-100” → “0.0-1.0”
            try:
            ...
            if query.lower() in text.lower():
                ...
                LOGGER.debug(
                    "Found text '%s' (conf=%.2f) at (%d,%d), clicking.",
                    text,
                    conf,
                    center_x,
                    center_y,
                )
                human_move_and_click(center_x, center_y)
                return True

        # Short sleep to avoid hogging CPU
        time.sleep(0.3)

    LOGGER.debug("Text '%s' not found within %.1fs", query, timeout)
    return False
